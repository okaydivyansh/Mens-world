  ...
  if (inputAlphabet(firstname, "* For your name please use alphabets only *")) {
    if (lengthDefine(username, 6, 8)) {
      if (
        passwordV(
          password,
          "*please enter atleast one digit,one upper case character,one lower case character and one special character"
        )
      ) {
...
function passwordV(inputtext, alertMsg) {
  const passwordRegex = /^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[^\w\s]).{8,}$/;
  if (inputtext.value.match(passwordRegex)) {
    return true;
  } else {
    document.getElementById("p7").innerText = alertMsg; // This segment displays the validation rule for email.
    inputtext.focus();
    return false;
  }
}
